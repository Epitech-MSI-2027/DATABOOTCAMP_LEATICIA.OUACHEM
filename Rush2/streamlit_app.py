...
# ----------------------------- APP ------------------------------------
st.title("💊 Analytics Pharma (FR)")
st.caption("Filtres de période + KPI, **Pic & creux** (avec meilleur & pire code de la même période), **Tendances** (paramètres fixés).")
...
paths = {}
c1, c2, c3 = st.columns(3)
with c1: paths["Daily"]   = st.text_input("Chemin CSV — Quotidien (Daily)",    value=defaults["Daily"])
...
tab_daily, tab_weekly, tab_monthly = st.tabs(["Quotidien","Hebdomadaire","Mensuel"])
...
with tab_daily:
    ...
    if Path(p).exists():
        df = load_csv(p)
        df_f = filtre_daily_weekly(df, "Quotidien")
        ...
        if long.empty:
            ...
            kpi_block(long, "Quotidien", "jour")
            show_trends_fixed(long, "Daily", "Daily")
            top_bottom_block(long, "journée")
            courbe_temporelle(long, "Quotidien — Série temporelle")
            barres_par_code(long, "Quotidien — Totaux par code ATC")
    else:
